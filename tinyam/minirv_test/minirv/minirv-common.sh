...
if [[ "$src" == *.S ]] then
  ...
  ${TOOLPREFIX}$cc $flags -S -o $dst_S $src
fi
...
minirv_path=$AM_HOME/minirv_test/minirv
...
src_dir=`dirname $src`
${TOOLPREFIX}gcc -I$src_dir $flags -D_LUT_BIN_PATH=\"$lut_bin_path\" -Wno-trigraphs -c -o $dst $dst_S
